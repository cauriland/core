import { DatabaseService, Repositories } from "@cauriland/core-database";
import { Container, Contracts, Enums, Providers, Types, Utils } from "@cauriland/core-kernel";
import { DatabaseInteraction } from "@cauriland/core-state";
import { Blocks, Crypto, Interfaces, Managers } from "@cauriland/crypto";
...
export class Blockchain implements Contracts.Blockchain.Blockchain {
    ...
    @Container.tagged("plugin", "@cauriland/core-blockchain")
    ...
    public async initialize(): Promise<void> {
        ...
        if (this.stateStore.getNetworkStart()) {
            this.logger.warning(
                "Cauri Core is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong.",
            );
        }
