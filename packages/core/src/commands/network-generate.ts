...
import { Blocks, Crypto, Identities, Interfaces, Managers, Transactions, Utils } from "@cauriland/crypto";
...
export class Command extends Commands.Command {
    ...
    private flagSettings: Flag[] = [
        ...
        { name: "coreP2PPort", description: "Core P2P port.", schema: Joi.number(), default: 4300 },
        { name: "coreAPIPort", description: "Core API port.", schema: Joi.number(), default: 4303 },
        { name: "coreWebhooksPort", description: "Core Webhooks port.", schema: Joi.number(), default: 4304 },
        { name: "coreMonitorPort", description: "Core Webhooks port.", schema: Joi.number(), default: 4305 },
    ...
    private generateApp(options: Options): any {
        ...
        if (includeDynamicFees) {
            app.core.plugins.find((plugin) => plugin.package === "@cauriland/core-transaction-pool").options = {
                dynamicFees,
            };

            app.relay.plugins.find((plugin) => plugin.package === "@cauriland/core-transaction-pool").options = {
                dynamicFees,
            };
        }
