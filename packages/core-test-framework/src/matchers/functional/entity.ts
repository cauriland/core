import { Interfaces as MagistrateInterfaces } from "@cauriland/core-magistrate-crypto";
...
expect.extend({
    entityRegistered: async (transaction: Interfaces.ITransactionData) => {
        ...
        let errors;
        try {
            const { body } = await got.get(`http://localhost:4303/api/entities/${transaction.id}`);
    ...
    entityResigned: async (registrationId: string) => {
        ...
        let errors;
        try {
            const { body } = await got.get(`http://localhost:4303/api/entities/${registrationId}`);
    ...
    entityUpdated: async (updateTransaction: Interfaces.ITransactionData) => {
        ...
        try {
            ...
            const { body } = await got.get(`http://localhost:4303/api/entities/${entityAsset.registrationId}`);
