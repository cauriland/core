...
const main = async () => {
    let {
        ...
        type: 'text',
        name: 'corePath',
        initial: expandHomeDir('~/cauri-core'),
    ...
    // Paths
    const corePaths = envPaths('cauri', {
        suffix: 'core'
    });
    ...
    fs.writeFileSync(`${paths.config.new}/.env`, envCurrent.replace(new RegExp('CAURI_', 'g'), 'CORE_'));

    // Update plugins file
    console.log('Update plugins configuration');
    let pluginContents = fs.readFileSync(`${paths.config.new}/plugins.js`).toString();
    pluginContents = pluginContents.replace('@cauriland/core-transaction-pool-mem', '@cauriland/core-transaction-pool');
    pluginContents = pluginContents.replace('"@cauriland/core-config": {},', '');
    pluginContents = pluginContents.replace("'@cauriland/core-config': {},", '');
    pluginContents = pluginContents.replace(new RegExp('CAURI_', 'g'), 'CORE_');
