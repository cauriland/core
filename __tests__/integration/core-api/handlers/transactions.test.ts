...
import { Identities, Managers } from "@cauriland/crypto";
...
beforeAll(async () => {
    ...
    senderAddress = Identities.Address.fromPublicKey(genesisTransaction.senderPublicKey, 28);
...
describe("API 2.0 - Transactions", () => {
    describe("GET /transactions", () => {
        ...
        it("should GET transactions with the exact specified type", async () => {
            ...
            expect(response).toBeSuccessfulResponse();
            expect(response.data.data).toBeArray();
            expect(response.data.data).toHaveLength(101);
