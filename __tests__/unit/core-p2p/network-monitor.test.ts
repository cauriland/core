...
describe("NetworkMonitor", () => {
    ...
    describe.each([[true], [false]])("boot", (dnsAndNtpFail) => {
        ...
        describe("when peer discovery is disabled", () => {
            ...
            it("should populate peers from seed peers config by calling validateAndAcceptPeer", async () => {
                appConfigPeers.list = [
                    { ip: "187.177.54.44", port: 4300 },
                    { ip: "188.177.54.44", port: 4300 },
                    { ip: "189.177.54.44", port: 4300 },
                ];
            ...
            it("should populate peers from URL config by calling validateAndAcceptPeer", async () => {
                ...
                const peers = [
                    ...
                    { ip: "188.177.54.44", port: 4300 },
                    { ip: "189.177.54.44", port: 4300 },
                    { ip: "190.177.54.44", port: 4300 },
                    { ip: "191.177.54.44", port: 4300 },
                ];
            ...
            it("should populate peers only once if same peer is in list and sources", async () => {
                ...
                const peers = [{ ip: "187.177.54.44", port: 4300 }];

                appConfigPeers.list = [{ ip: "187.177.54.44", port: 4300 }];
        ...
        describe("when peer discovery is enabled", () => {
            ...
            it("should discover peers from seed peers (calling updateNetworkStatus) and log the peers discovered by version", async () => {
                ...
                const peers = [
                    { ip: "187.177.54.44", port: 4300, version: "3.0.0" },
                    { ip: "188.177.54.44", port: 4300, version: "3.0.0" },
                    { ip: "189.177.54.44", port: 4300, version: "3.0.1" },
                    { ip: "190.177.54.44", port: 4300, version: "3.0.2" },
                    { ip: "191.177.54.44", port: 4300, version: "3.0.2" },
                ];
    ...
    describe("cleansePeers", () => {
        const peers = [
            new Peer("187.177.54.44", 4300),
            new Peer("188.177.54.44", 4300),
            new Peer("189.177.54.44", 4300),
            new Peer("190.177.54.44", 4300),
            new Peer("191.177.54.44", 4300),
        ];
        beforeEach(() => {
    ...
    describe("discoverPeers", () => {
        const peers = [
            new Peer("180.177.54.4", 4300),
            new Peer("181.177.54.4", 4300),
            new Peer("182.177.54.4", 4300),
            new Peer("183.177.54.4", 4300),
            new Peer("184.177.54.4", 4300),
            new Peer("185.177.54.4", 4300),
            new Peer("186.177.54.4", 4300),
            new Peer("187.177.54.4", 4300),
            new Peer("188.177.54.4", 4300),
            new Peer("189.177.54.4", 4300),
        ];
        beforeEach(() => {
            repository.getPeers = jest.fn().mockReturnValue(peers);
        });
    ...
    describe("getNetworkHeight", () => {
        it.each([
            ...
            for (let i = 0; i < peersHeights.length; i++) {
                const peer = new Peer(`188.185.1.${i}`, 4300);
    ...
    describe("downloadBlocksFromHeight", () => {
        ...
        it("should return empty array and log an error when all our peers are forked", async () => {
            const peer = new Peer("1.1.1.1", 4300);
        ...
        it("should download blocks in parallel from N peers max", async () => {
            ...
            for (let i = 0; i < maxParallelDownloads + 5; i++) {
                const peer = new Peer(`1.1.1.${i}`, 4300);
