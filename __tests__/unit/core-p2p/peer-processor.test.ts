import { Container, Enums } from "@cauriland/core-kernel";

import { PeerProcessor } from "@cauriland/core-p2p/src/peer-processor";
...
describe("PeerProcessor", () => {
    ...
    const peerFactory = (ip) => new Peer(ip, 4300);
    ...
    describe("validateAndAcceptPeer", () => {
        it("should accept a new peer if its ip is validated", async () => {
            const peer = new Peer("178.165.55.55", 4300);
    ...
    describe("validatePeerIp", () => {
        ...
        it("should return false when peer is not valid", () => {
            const invalidPeer = new Peer("127.0.0.1", 4300); // localhost is invalid
        ...
        it("should return false when there are already too many peers on the peer subnet and not in seed mode", () => {
            const sameSubnetPeers = [new Peer("178.165.55.50", 4300), new Peer("178.165.55.51", 4300)];
