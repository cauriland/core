...
import { DisconnectInvalidPeers, DisconnectPeer } from "@cauriland/core-p2p/src/listeners";
...
describe("DisconnectPeer", () => {
    ...
    describe("handle", () => {
        it("should disconnect the peer provided", async () => {
            const peer = new Peer("187.176.1.1", 4300);
            await disconnectPeer.handle({ data: { peer: peer, port: 4300 } });
