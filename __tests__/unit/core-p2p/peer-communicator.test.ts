...
import { Container, Utils as KernelUtils } from "@cauriland/core-kernel";
import { constants } from "@cauriland/core-p2p/src/constants";
import {
    PeerPingTimeoutError,
    PeerStatusResponseError,
    PeerVerificationFailedError,
} from "@cauriland/core-p2p/src/errors";
import { Peer } from "@cauriland/core-p2p/src/peer";
import { PeerCommunicator } from "@cauriland/core-p2p/src/peer-communicator";
import { PeerVerificationResult } from "@cauriland/core-p2p/src/peer-verifier";
import { replySchemas } from "@cauriland/core-p2p/src/schemas";
import { Blocks, Identities, Managers, Transactions, Utils } from "@cauriland/crypto";
import delay from "delay";

jest.mock("@cauriland/core-p2p/src/socket-server/utils/get-codec", () => ({
...
describe("PeerCommunicator", () => {
    ...
    describe("postBlock", () => {
        it("should use connector to emit p2p.blocks.postBlock", async () => {
            ...
            const payload = { block };
            const peer = new Peer("187.168.65.65", 4300);
    ...
    describe("getPeers", () => {
        it("should use connector to emit p2p.peer.getPeers", async () => {
            ...
            const mockConnectorResponse = { payload: [{ ip: "177.176.1.1", port: 4300 }] };
