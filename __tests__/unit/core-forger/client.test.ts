...
import { Client } from "@cauriland/core-forger/src/client";
import { Application, Container } from "@cauriland/core-kernel";
import { NetworkStateStatus, Nes, Codecs } from "@cauriland/core-p2p";
...
describe("Client", () => {
    ...
    const host = { hostname: "127.0.0.1", port: 4300, socket: undefined };
    const hostIPv6 = { hostname: "::1", port: 4300, socket: undefined };
    const hosts = [host];
    ...
    describe("dispose", () => {
        it("should call disconnect on all sockets", () => {
            client.register([host, { hostname: "127.0.0.5", port: 4300 }]);
            client.dispose();
    ...
    describe("syncWithNetwork", () => {
        ...
        it("should log error message if syncing fails", async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith(
                `Could not sync check: Request to 127.0.0.1:4300<p2p.internal.syncBlockchain> failed, because of '${errorMessage}'.`,
            );
        });
    });
