...
describe("getLatestVersion", () => {
    it("should fail to get the npm registry channel", async () => {
        await expect(getLatestVersion("@cauriland/core", "latest")).resolves.toBeUndefined();
    });

    it("should get the npm registry channel (latest)", async () => {
        nock(/.*/).get("/@cauriland%2Fcore").reply(200, versionLatest);

        await expect(getLatestVersion("@cauriland/core", "latest")).resolves.toBe("2.5.24");
    });

    it("should get the npm registry channel (next)", async () => {
        nock(/.*/).get("/@cauriland%2Fcore").reply(200, versionNext);

        await expect(getLatestVersion("@cauriland/core", "next")).resolves.toBe("2.5.0-next.10");
    });
});
...
describe("installFromChannel", () => {
    it("should be ok if no [stderr] output is present", () => {
        ...
        installFromChannel("@cauriland/core", "latest");

        expect(spySync).toHaveBeenCalledWith("yarn global add @cauriland/core@latest");
    });
    ...
    it("should be not ok if [stderr] output is present", () => {
        ...
        expect(() => installFromChannel("@cauriland/core", "latest")).toThrow("stderr");
