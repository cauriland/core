...
describe("Git", () => {
    describe("#exists", () => {
        it("should return true if the file exists", async () => {
            await expect(source.exists("git@github.com:cauriland/utils.git")).resolves.toBeTrue();
        });
    ...
    describe("#install", () => {
        it("should successfully install the plugin", async () => {
            ...
            const spyOnExeca = jest.spyOn(execa, "sync").mockImplementation(() => {
                ...
                fs.writeJSONSync(join(tempPath, "package", "package.json"), { name: "@cauriland/utils" });
            });

            // Act
            const repository: string = "git@github.com:cauriland/utils.git";
