...
describe("CLI", () => {
    ...
    it("should set exitCode = 2 when using invalid commands", async () => {
        ...
        expect(message).toContain(`is not a cauri command.`);
    ...
    describe("discover plugins", () => {
        ...
        it("should load CLI plugins from folder using detected network folder", async () => {
            ...
            expect(spyOnList).toHaveBeenCalledWith("cauri", "testnet");
            expect(spyOnDiscoverNetwork).toHaveBeenCalledWith(envPaths("cauri", { suffix: "core" }).config);
        });
