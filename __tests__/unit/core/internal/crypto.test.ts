...
describe("buildBIP38", () => {
    ...
    it("should throw if the delegate configuration does not exist", async () => {
        await expect(buildBIP38({ token: "cauri", network: "mainnet" })).rejects.toThrow(
    ...
    it("should use the bip38 from the delegate configuration", async () => {
        ...
        const { bip38, password } = await buildBIP38({ token: "cauri", network: "mainnet" });
    ...
    it("should throw if no bip38 password is provided and skipPrompts is true", async () => {
        ...
        await expect(buildBIP38({ token: "cauri", network: "mainnet", skipPrompts: true })).rejects.toThrow(
    ...
    it("should throw if no bip38 password is provided", async () => {
        ...
        await expect(buildBIP38({ token: "cauri", network: "mainnet" })).rejects.toThrow(new Crypto.InvalidPassword());
    });
