...
test("TransactionPoolFullError", () => {
    ...
    expect(error.message).toBe(`${transaction} fee 0.000009 Dꚦ is lower than 0.00001 Dꚦ already in pool`);
});
...
test("TransactionFromWrongNetworkError", () => {
    const error = new TransactionFromWrongNetworkError(transaction, 28);

    expect(error).toBeInstanceOf(Contracts.TransactionPool.PoolError);
    expect(error.type).toBe("ERR_WRONG_NETWORK");
    expect(error.message).toBe(`${transaction} network 30 doesn't match node's network 28`);
});
