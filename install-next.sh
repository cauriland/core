...
heading "Installing Cauri Core..."

while ! yarn global add @cauriland/core@next ; do
    read -p "Installing Cauri Core failed, do you want to retry? [y/N]: " choice
...
cauri config:publish --network=devnet

success "Installed Cauri Core!"
...
if [[ "$choice" =~ ^(yes|y|Y) ]]; then
    ...
    cauri env:set --key=CORE_DB_USERNAME --value="${databaseUsername}"
    cauri env:set --key=CORE_DB_PASSWORD --value="${databasePassword}"
    cauri env:set --key=CORE_DB_DATABASE --value="${databaseName}"
